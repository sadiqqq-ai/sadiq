...
if(isset($_POST['processOrder'])){
    if(isset($_SESSION['userid'])){
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $sql = "INSERT INTO orders (userid, products,date_ordered)
            VALUES ('$userid', '$products', DATE(NOW()))";
...
<head>
    <meta charset="UTF-8">
    <title>Payment Gateway - A & A</title>
...
<body>
    ...
    <div class="product" style="padding: 20px;">
        <div class="product-details">
            <div class=" product-title"><b><?php echo $row['product_name']?></b></div>
        </div>
        <div class="product-price">Price: AED<?php echo $row['price'];?></div>
        <div class="product-quantity">
            Quantity:<?php echo $row['cQuantity']?>
        </div>

        <div class="product-line-price">
            AED<?php echo (preg_replace('/[^\d.]/', '',$row['price'])*$row['cQuantity']);?>
        </div>
    </div>
    <br>
    <?php }}
            ...
            echo '<div style="width:100%; text-align:center"> User not logged in</div>';
        }?>
    <!-- partial:index.partial.html -->
    <div class="payment-title">
        <h1>A & A Payment Information</h1>
        <a href="cart.php" class="checkout-btn">Back to Cart</a>
        <form method="POST">
            <button name='processOrder' class="checkout-btn">Process Order</button>
        </form>
    </div>
